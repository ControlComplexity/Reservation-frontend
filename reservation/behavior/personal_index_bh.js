	...
	methods: {
		...
		_loadTodayList: async function () {
			try {
				...
				await cloudHelper.callCloudSumbit('my/personal_center_join_someday', params, opts).then(res => {
					this.setData({
		...
		_loadUser: async function (e) {
			...
			let user = await cloudHelper.callCloudData('passport/personal_center_detail', {}, opts);
			if (!user) return;
